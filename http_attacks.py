...
from aioquic.h3.connection import H3_ALPN, ErrorCode, H3Connection, encode_frame, FrameType, encode_settings, Setting, StreamType, encode_uint_var
...
# Adjustment: At this point, overridden functions and classes are imported from Attacker.py.
from Attacker import send_headers_settings, H3ConnectionChild, send_goaway_corrupt, send_headers_corrupt, send_settings_corrupt, send_data_without_check
...
# Adjustment: As part of the adjustment, a class was overwritten, which is added at this point.
HttpConnection = Union[H0Connection, H3Connection, H3ConnectionChild]

USER_AGENT = "aioquic/" + aioquic.__version__

# Adjustment: As part of the adjustment, classes were overwritten for which global variables are defined at this point. 
...
# Adjustment: Send attack depending on test procedure.
def Testcases(self) -> None:
        ...
        # Wrong Frames for T9
        if WRONG_FRAMES["controlstream-data"] == True:
            self.send_data(self._local_control_stream_id, bytes(0), False)

        if WRONG_FRAMES["controlstream-headers"] == True:
...
# Adjustment: Send attack depending on test procedure.
async def _requestTest(self, request: HttpRequest) -> Deque[H3Event]:
    global REQUEST_STREAM_ID
    if REQUEST_STREAM_ID == 999:
        stream_id = self._quic.get_next_available_stream_id()
...
# Adjustment: Monkey Patch HttpClient with Attack Functions.
class HTTPClientMonkey(HttpClient):
        def __init__(self, *args, **kwargs) -> None:
            ...
            # Monkeypatch our Testcases!
            H3Connection._init_connection = Testcases
            HTTPClientMonkey._request = _requestTest
            H3Connection.send_data = send_data_without_check
...
# Adjustment: A Client with a adjusted H3Connection to perform tests.
class HttpClientCorrupt(HttpClient):
    def __init__(self, *args, **kwargs) -> None:
        super(HttpClient,  self).__init__(*args, **kwargs)
...
async def main(
    ...
    # Adjustment: Starting attack by using patched HttpClientMonkey or HttpClientCorrupt.
    ...
    for i in range (0, 2):
        print("\r\n")
        print("-" * 27)
        print("-" * 4, "Starting Test T", str(i) , "-" * 4)
        print("-" * 27, "\r\n")
        ...
        SETTINGS_VALUE = 2048
        LENGTH_OFFSET = 0
        WRONG_FRAMES = {"none": False, "requeststream-settings": False, "requeststream-goaway": False, "controlstream-data": False, "controlstream-headers": False, "requeststream-doublerequest": False}

        if i == 0 and test != 0:
            print("Test: Sending Frames on not supported Streams")

            key = list(WRONG_FRAMES.keys())[test]
            WRONG_FRAMES[key] = True
            print("\r\nTesting Stream-Frame Combo: ", key,"\r\n")
            ...
            try:
                async with connect(
                    ...
                    port,
                    configuration=configuration,
                    create_protocol=HTTPClientMonkey,
                    session_ticket_handler=save_session_ticket,
                    local_port=local_port,
                    wait_connected=not zero_rtt,
                ) as client:
                    client = cast(HTTPClientMonkey, client)
                    z = 1
                    if test == 4:
                        z = 2
                    for  i in range(0,z):
        ...
        elif i == 1 and offset_test == True:
            list_offsets = [-1, -8, -16]
            print("Manipulating Frame Size with OFFSET that doesnt equal content-length header", "\r\n")

            for value in list_offsets:
                try:
                    print("\r\nTesting Offset: ", value, "\r\n")
                    LENGTH_OFFSET = value
                    async with connect(
                        host,
                        port,
                        configuration=configuration,
                        create_protocol=HttpClientCorrupt,
                        session_ticket_handler=save_session_ticket,
                        local_port=local_port,
                        wait_connected=not zero_rtt,
                    ) as client:
                        client = cast(HttpClientCorrupt, client)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", type=int, default=0, help="test frame on a invalid stream (0-5)"
    )
    parser.add_argument(
        "--offset-test",
        action="store_true",
        help="send combinations of a invalid frame length and content-length header",
    )
    ...
    if args.session_ticket:
        try:
            ...
            pass
    if args.offset_test == True and args.t != 0:
        raise Exception("Cant use this combination of args")
